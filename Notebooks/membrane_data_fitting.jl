...
md"""
#### 1: Fit RhoA negative feedback model to membrane NON-KO data:
"""
...
begin
	...
	rhoa_data_NONFA = median_rhoa_NONFA[16:length(median_rhoa_NONFA)]
	rhoa_data_NONFA_KO = median_rhoa_NONFA_KO[16:length(median_rhoa_NONFA_KO)]
end
...
function MSE_loss(new_parameters)
	...
	if length(new_sol_array) == length(rhoa_data_NONFA)
		loss = sum(abs2, new_sol_array .- rhoa_data_NONFA) + abs(new_sol[RhoA_model.aGEF][1] - new_sol[RhoA_model.aGEF][length(new_sol_array)]) + abs(new_sol[RhoA_model.aGAP][1] - new_sol[RhoA_model.aGAP][length(new_sol_array)])
		#
	else
		loss = 1000
	end
...
begin
	...
	scatter(timeframe, rhoa_data_NONFA, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
...
begin
	saved_parameters = [0.0221484, 0.146819, 0.119009, 0.295593, 0.0292401, 0.0224221, 0.144473, 0.0549748, 0.161294, 0.0309768, 0.715132, 1.02328]
...
@mtkmodel RhoA_KO_dynamics begin
	...
	@equations begin
		D(aGEF) ~ 0.0549748 * S - Kinact_GEF * aGEF + Kact_GEF
		D(aGAP) ~ Kcat_FL * ((10 - aGAP) * aRhoA)/(Km_FL + (10 - aGAP)) - Kinact_GAP * aGAP + Kact_GAP
		D(aRhoA) ~ 0.0292401 * aGEF * (10 - aRhoA)/(0.119009 + (10 - aRhoA)) - (Kcat_aGAP * aGAP * aRhoA/(Km_aGAP + aRhoA))
		S ~ f_tanh(t)
...
function KO_MSE_loss(new_parameters)
	...
	# Determine the loss by computing the sum of squares¨
	if length(KO_new_sol_array) == length(rhoa_data_NONFA_KO)
		loss = sum(abs2, KO_new_sol_array .- rhoa_data_NONFA_KO) + (abs2(KO_new_sol[RhoA_KO_model.aGEF][1] - KO_new_sol[RhoA_KO_model.aGEF][length(KO_new_sol_array)]) + abs2(KO_new_sol[RhoA_KO_model.aGAP][1] - KO_new_sol[RhoA_KO_model.aGAP][length(KO_new_sol_array)]))
...
# ╔═╡ b1ad2a0d-7407-4259-944f-b37efb9af9f3
RhoA_KO_prob.p
...
begin
	...
	KO_pred_sol = solve(remake(RhoA_KO_prob, u0=KO_predicted_u0, p=KO_predicted_parameters), saveat=1, dtmax=0.5)
	scatter(timeframe, rhoa_data_NONFA_KO, label = "Data", lw=2, title = "Membrane RhoA dynamics in DLC1-KO cells")
	plot!(KO_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
end
  ╠═╡ =#

# ╔═╡ cd1e1c67-1fd4-449e-b1bc-2e79d019901e
RhoA_KO_prob.p

# ╔═╡ 259c7f4b-fe37-48ad-a5a9-640f460e1dc8
begin
	saved_KO_parameters = [0.00142813, 0.192837, 0.0232547, 0.0629895, -0.00786548, 0.0215083, 0.234168, 0.00213048, 1.0052, 0.555346]
	...
	scatter(timeframe, rhoa_data_NONFA_KO, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
...
begin
	...
	scatter!(timeframe, rhoa_data_NONFA_KO, label = "DLC1-KO Data", lw=2, title = "Membrane RhoA dynamics", markercolor="blue")
	plot!(saved_pred_sol, lw=2, idxs=3, linecolor="red",label = "WT aRhoA")
	scatter!(timeframe, rhoa_data_NONFA, label = "WT Data", markercolor="red3")
	xlabel!("Time [s]")
