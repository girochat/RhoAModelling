...
using DifferentialEquations, Optimization, SciMLSensitivity, ModelingToolkit, Optim, OptimizationPolyalgorithms

# ╔═╡ 7f53f00a-f877-4dbb-901d-572d0bd6c54a
using ForwardDiff, Plots

# ╔═╡ e6ac7a43-97ea-45a5-9950-b29b2a0e89b9
using DelimitedFiles

# ╔═╡ a97176f9-7dd2-4870-9a80-b91e25ba8b97
md"""
## Data fitting of FA RhoA activation/deactivation dynamics
This notebook contains the code used to fit an ODE model to the RhoA dynamics data obtained after local light stimulation at focal adhesions as described in the paper by Heydasch et al.  \
...
begin
	...
	# Define the light signal function (hyperbolic tangent as continuous approximation)

	#  of light signal function to have continuous derivatives for optimisation 
...
begin
	x = LinRange(148, 203, 200)
	y = f_tanh.(x)
	plot(x, y, labels ="Tanh", lw = 2, title="Comparison of light signal functions")
end
...
@mtkmodel RhoA_FA_dynamics begin
	...
	@parameters begin
		...
		Kcat_FL
		Km_FL # found to be set to 0 after optimisation

		Kcat_aGAP
		Km_aGAP # found to be set to 0 after optimisation
	...
	@equations begin
		D(aGEF) ~ Kinact_GEF * (GEF0 * (S) - aGEF) + Kcat_PFL * ((aRhoA-1)*(tanh(100*(aRhoA-1))-tanh(100*(aRhoA-10)))/2 * (10 - aGEF))/(Km_PFL + (10 - aGEF))
		D(aGAP) ~ Kcat_FL * ((aRhoA-1)*(tanh(100*(aRhoA-1))-tanh(100*(aRhoA-10)))/2 * (10 - aGAP))/(Km_FL + (10 - aGAP)) + Kinact_GAP * (GAP0-aGAP)
		D(aRhoA) ~ Kcat_aGEF * (aGEF-GEF0) * (10 - aRhoA)/(Km_aGEF + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0) * (aRhoA)/(Km_aGAP + (aRhoA)) 
		S ~ f_tanh(t)
	end
end

# ╔═╡ eb81d398-90e5-4d4e-bab5-3df9ac5df099
@mtkbuild RhoA_FA_model = RhoA_FA_dynamics()

# ╔═╡ 05840396-3ee2-46d1-99aa-dda4c209a39f
begin

	# Define some initial parameters for the ODE model
	FA_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.2, 0.21, 0.22, 0.5, 0.5]

	# Build ODE problems with guess parameters and initial conditions
	RhoA_FA_prob = ODEProblem(RhoA_FA_model, 
		[RhoA_FA_model.aGEF => 0.5, 
			RhoA_FA_model.aRhoA => 1,
			RhoA_FA_model.aGAP => 0.5
		], 
		(140.0, 400.0), 
		FA_parameter_guess[1:12], jac = true)
end

# ╔═╡ ebeb1daa-d442-4141-a2d5-b3351fb70b8d
RhoA_FA_prob.p
...
@mtkmodel RhoA_FA_KO_dynamics begin
	...
	@equations begin
		D(aGEF) ~ 0.64 * (GEF0 * (S) - aGEF) + 2.02 * 
		((aRhoA-1)*(tanh(100*(aRhoA-1))-tanh(100*(aRhoA-10)))/2 * (10 - aGEF))/(2.06 + (10 - aGEF))
		D(aGAP) ~ Kcat_FL * ((aRhoA-1)*(tanh(100*(aRhoA-1))-tanh(100*(aRhoA-10)))/2 * (10 - aGAP))/(Km_FL + (10 - aGAP)) + Kinact_GAP * (GAP0-aGAP)
		D(aRhoA) ~ 0.065 * (aGEF-GEF0) * (10 - aRhoA)/(0 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0) * (aRhoA)/(Km_aGAP + (aRhoA))
		S ~ f_tanh(t)
...
@mtkbuild RhoA_FA_KO_model = RhoA_FA_KO_dynamics()

# ╔═╡ 51218007-1366-4efc-be76-8596f1a6384c
begin

	# Define some initial parameters for the ODE model
	FA_KO_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.5, 0.5]  

	# Build ODE problems with guess parameters and initial conditions
	RhoA_FA_KO_prob = ODEProblem(RhoA_FA_KO_model, 
		[RhoA_FA_KO_model.aGEF => 0.5, 
			RhoA_FA_KO_model.aRhoA => 1, 
			RhoA_FA_KO_model.aGAP => 0.5
		], 
		(150.0, 400.0), 
		FA_KO_parameter_guess[1:7], jac = true)
end
...
begin 
	...
	if length(FA_KO_track_loss) > 0
		deleteat!(FA_KO_track_loss, 1:length(FA_KO_track_loss))
	end

	# Run the optimisation
	FA_KO_optim_results = Optimization.solve(FA_KO_optimisation_prob, LBFGS(), callback = FA_KO_callback!, maxiters = 100, progress = true)
end
  ╠═╡ =#
...
begin
	...
	GEF0_KO = FA_KO_optim_results.u[8]
	GAP0_KO = FA_KO_optim_results.u[9]
	FA_KO_predicted_u0 = [GEF0; GAP0; [1]]

	# Re-modify parameters as for optimisation
	FA_KO_pred_p = FA_KO_optim_results[1:7]
	FA_KO_pred_p[get_idxparam(RhoA_FA_KO_model.GEF0, RhoA_FA_KO_prob)] = GEF0_KO
	FA_KO_pred_p[get_idxparam(RhoA_FA_KO_model.GAP0, RhoA_FA_KO_prob)] = GAP0_KO 

	# Solve the ODE problem with optimal parameters
	FA_KO_pred_sol = solve(remake(RhoA_FA_KO_prob, p=FA_KO_pred_p, u0=[GEF0_KO; GAP0_KO; 1], tspan=(120, 400)), saveat=0.5, dtmax = 0.05)

	scatter(timeframe[12:end], median_rhoa_FA_KO[12:end], label = "Data", lw=2, title = "RhoA dynamics in FA and DLC1-KO cells")
	plot!(FA_KO_pred_sol, label = "Prediction", lw=2, idxs=3)
...
# Define MSE loss function to estimate parameters for FA and WT RhoA data
function FA_MSE_loss(new_parameters)

	# Substitute the initial conditions in the parameters
	new_GEF0 = new_parameters[13]
	new_GAP0 = new_parameters[14]
	new_u0 = [new_GEF0; new_GAP0; 1]
	new_p = new_parameters[1:12]       # (it performs a deep copy)
	new_p[get_idxparam(RhoA_FA_model.GEF0, RhoA_FA_prob)] = new_GEF0
	new_p[get_idxparam(RhoA_FA_model.GAP0, RhoA_FA_prob)] = new_GAP0
	
	# Update the ODE problem with the new set of parameters
	FA_new_ODE_prob = remake(RhoA_FA_prob, p = new_p, u0 = new_u0)

	# Solve the new ODE problem (saveat = numerical estimate time interval)
	FA_new_sol = solve(FA_new_ODE_prob, AutoVern9(Rodas5P()), saveat = 10, verbose = false, maxiters = 1e7, dtmax=0.05)
	FA_new_sol_array = FA_new_sol[RhoA_FA_model.aRhoA]

	# Determine the loss by computing the sum of squares
	if length(FA_new_sol_array) == length(median_rhoa_FA[14:end])
		loss = sum(abs2, FA_new_sol_array .- median_rhoa_FA[14:end])
	else
		loss = 1000
	end

	return loss, FA_new_sol
end

# ╔═╡ 4b34bb5f-1be5-47a1-a2d7-677a86e7503b
begin
	# Define optimization function and differentiation method
	FA_optimisation_f = Optimization.OptimizationFunction((x, p) -> FA_MSE_loss(x), Optimization.AutoForwardDiff())

	# Keep track of loss values during iterations
	FA_track_loss = []

	# Set upper and lower bound for the parameters and initial conditions
	FA_lower_b = repeat([0], 14)
	FA_upper_b = [repeat([10], 12); 2; 2]

	# Define the optimisation problem
	FA_optimisation_prob = Optimization.OptimizationProblem(FA_optimisation_f, FA_parameter_guess, lb = FA_lower_b, ub=FA_upper_b)

end

# ╔═╡ fe29f3cc-2f90-4c33-b70f-d8ba91e950ea
# Define callback function that is run at each iteration
FA_callback! = function(parameters, loss, new_sol)

	# Add current loss value to tracking array
	push!(FA_track_loss, loss)
...
begin 
	...
	# Clear the tracking array if not empty
	if length(FA_track_loss) > 0
		deleteat!(FA_track_loss, 1:length(FA_track_loss))
	end
	
	# Run the optimisation
	FA_optim_results = Optimization.solve(FA_optimisation_prob, BFGS(), callback = FA_callback!, maxiters = 10, progress = true)
end
...
plot(FA_track_loss, title = "Evolution of the loss value", label = false)

# ╔═╡ f31fa805-d0f8-4f04-abaf-d17e0483350e
begin
	# Retrieve initial conditions from optimisation solution
	GEF0 = FA_optim_results.u[13]
	GAP0 = FA_optim_results.u[14]
	FA_pred_u0 = [GEF0; GAP0; 1]

	# Re-modify parameters as for optimisation
	FA_pred_p = FA_optim_results[1:12]
	FA_pred_p[get_idxparam(RhoA_FA_model.GEF0, RhoA_FA_prob)] = GEF0
	FA_pred_p[get_idxparam(RhoA_FA_model.GAP0, RhoA_FA_prob)] = GAP0

	# Solve the ODE problem with optimal parameters
	FA_pred_sol = solve(remake(RhoA_FA_prob, p=FA_pred_p, u0=FA_pred_u0, tspan=(120, 400)), saveat=0.5, dtmax = 0.05) 
	...
	plot!(FA_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
end

# ╔═╡ e1c81eb4-9a0b-4039-802c-c110c688151b
plot(FA_pred_sol, label = "Prediction", lw=2, idxs=1, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")

# ╔═╡ 21f8b5e7-056e-4ea6-abc5-d23d7c8bddfe
plot(FA_pred_sol, title = "aGEF, aGAP and aRhoA simultaneous dynamics", xlabel = "Time [s]")#, idxs=2)

# ╔═╡ ceea52cf-7b52-44a1-af60-127c0eb76bde
begin
	# Save best parameters
	saved_FA_parameters = [0.0658527, 7.01907, 0.643158, 0, 1.87397, 0.139773, 0.0975044, 2.06477, 1.53833, 2.01886, 4.60078, 10.0, 0.50094, 1.90924]  

	# Transform back ODE parameters to their symbolic expression
	saved_FA_param = [
		RhoA_FA_model.Kinact_GEF => saved_FA_parameters[get_idxparam(RhoA_FA_model.Kinact_GEF, RhoA_FA_prob)],
		RhoA_FA_model.Kcat_PFL => saved_FA_parameters[get_idxparam(RhoA_FA_model.Kcat_PFL, RhoA_FA_prob)],
		RhoA_FA_model.Km_PFL => saved_FA_parameters[get_idxparam(RhoA_FA_model.Km_PFL, RhoA_FA_prob)],
		RhoA_FA_model.Kcat_aGEF => saved_FA_parameters[get_idxparam(RhoA_FA_model.Kcat_aGEF, RhoA_FA_prob)], 
		RhoA_FA_model.Km_aGEF => saved_FA_parameters[get_idxparam(RhoA_FA_model.Km_aGEF, RhoA_FA_prob)],
		RhoA_FA_model.Kinact_GAP => saved_FA_parameters[get_idxparam(RhoA_FA_model.Kinact_GAP, RhoA_FA_prob)],
		RhoA_FA_model.Kcat_FL => saved_FA_parameters[get_idxparam(RhoA_FA_model.Kcat_FL, RhoA_FA_prob)],
		RhoA_FA_model.Km_FL => saved_FA_parameters[get_idxparam(RhoA_FA_model.Km_FL, RhoA_FA_prob)],
		RhoA_FA_model.Kcat_aGAP => saved_FA_parameters[get_idxparam(RhoA_FA_model.Kcat_aGAP, RhoA_FA_prob)],
		RhoA_FA_model.Km_aGAP => saved_FA_parameters[get_idxparam(RhoA_FA_model.Km_aGAP, RhoA_FA_prob)],
		RhoA_FA_model.GEF0 => saved_FA_parameters[13],
		RhoA_FA_model.GAP0 => saved_FA_parameters[14]
	]
	saved_FA_u0 = [saved_FA_parameters[13:14]; 1]

	# Solve the ODE with the best parameters and plot
	saved_FA_pred_sol = solve(remake(RhoA_FA_prob, p=saved_FA_param, u0=saved_FA_u0), saveat=1, dtmax=0.05, tspan=(120, 400)) 
	scatter(timeframe[12:40], median_rhoa_FA[12:40], label = "Data", lw=2, title = "FA RhoA dynamics in WT cells")
	plot!(saved_FA_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
...
function FA_KO_MSE_loss(new_parameters)

	# Substitute the initial conditions in the parameters
	new_u0 = [new_parameters[8:9]; [1]]
	new_p = new_parameters[1:7]
	new_p[get_idxparam(RhoA_FA_KO_model.GEF0, RhoA_FA_KO_prob)] = new_parameters[8]
	new_p[get_idxparam(RhoA_FA_KO_model.GAP0, RhoA_FA_KO_prob)] = new_parameters[9] 
	
	# Update the ODE problem with the new set of parameters
	FA_KO_new_ODE_prob = remake(RhoA_FA_KO_prob, p=new_p, u0=new_u0)

	# Solve the new ODE problem (saveat = numerical estimate time interval)
	FA_KO_new_sol = solve(FA_KO_new_ODE_prob, AutoVern9(Rodas4P()), saveat = 10, maxiters = 1e7, verbose = false, dtmax=0.05)
	FA_KO_new_sol_array = FA_KO_new_sol[RhoA_FA_KO_model.aRhoA]

	# Determine the loss (sum of squares)
	if length(FA_KO_new_sol_array) == length(median_rhoa_FA_KO[15:end])
		FA_loss = sum(abs2, FA_KO_new_sol_array .- median_rhoa_FA_KO[15:end])
	else
		FA_loss = 1000
	end

	return FA_loss, FA_KO_new_sol
end

# ╔═╡ 0a3ec77c-36f7-4571-b9bc-7f64c45c393c
begin
	# Define optimization function and differentiation method
	FA_KO_optimisation_f = Optimization.OptimizationFunction((x, p) -> FA_KO_MSE_loss(x), Optimization.AutoForwardDiff())

	# Keep track of loss values during iterations
	FA_KO_track_loss = []

	# Set upper and lower bound for the parameters and initial conditions
	FA_KO_lower_b = repeat([0], 9)
	FA_KO_upper_b = [repeat([10], 7); 2; 10]

	# Define the optimisation problem
	FA_KO_optimisation_prob = Optimization.OptimizationProblem(FA_KO_optimisation_f, FA_KO_parameter_guess, lb= FA_KO_lower_b, ub=FA_KO_upper_b)
end

# ╔═╡ d0af947c-168f-4ea1-aac9-b0117277b1aa
# Define callback function that is run at each iteration
FA_KO_callback! = function(parameters, loss, new_sol)

	# Add current loss value and parameter to tracking arrays
	push!(FA_KO_track_loss, loss)
...
# Plot the loss value at each iteration
plot(FA_KO_track_loss, title = "Evolution of the loss value", label = false)

# ╔═╡ f5877b34-cefe-4653-acd1-7ae9344e7d4f
begin
	# Save best parameters
	saved_FA_KO_parameters = [4.5021, 0.131124, 5.00055, 0.694472, 0.162417, 0, 5.00134, 0.533562, 2.47967]

	# Transform back ODE parameters to their symbolic expression
	saved_FA_KO_param = [
		RhoA_FA_KO_model.Kinact_GAP => saved_FA_KO_parameters[get_idxparam(RhoA_FA_KO_model.Kinact_GAP, RhoA_FA_KO_prob)],
		RhoA_FA_KO_model.Kcat_FL => saved_FA_KO_parameters[get_idxparam(RhoA_FA_KO_model.Kcat_FL, RhoA_FA_KO_prob)], 
		RhoA_FA_KO_model.Km_FL => saved_FA_KO_parameters[get_idxparam(RhoA_FA_KO_model.Km_FL, RhoA_FA_KO_prob)],
		RhoA_FA_KO_model.Kcat_aGAP => saved_FA_KO_parameters[get_idxparam(RhoA_FA_KO_model.Kcat_aGAP, RhoA_FA_KO_prob)],
		RhoA_FA_KO_model.Km_aGAP => saved_FA_KO_parameters[get_idxparam(RhoA_FA_KO_model.Km_aGAP, RhoA_FA_KO_prob)],
		RhoA_FA_KO_model.GEF0 => saved_FA_KO_parameters[8],
		RhoA_FA_KO_model.GAP0 => saved_FA_KO_parameters[9]]
	saved_FA_KO_u0 = [saved_FA_KO_parameters[8:9]; 1]

	# Solve the ODE with the best parameters and plot
	saved_FA_KO_pred_sol = solve(remake(RhoA_FA_KO_prob, p=saved_FA_KO_param, u0 = saved_FA_KO_u0, tspan=(120, 400)), saveat=1, dtmax=0.05) 
	scatter(timeframe[12:end], median_rhoa_FA_KO[12:end], label = "Data", lw=2, title = "FA RhoA dynamics in WT cells")
	plot!(saved_FA_KO_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
end
...
begin
	plot(saved_FA_KO_pred_sol, lw=2, idxs=3, linecolor="steelblue", label = "DLC1-KO (Prediction)")
	...
	plot!(saved_FA_pred_sol, lw=2, idxs=3, linecolor="red",label = "WT (Prediction)")
...
begin
	gef_plot = plot(saved_FA_pred_sol, title = "", xlabel = "", idxs=1, linecolor="red", linestyle=:dot, legend_position=:topleft)
	plot!(twinx(), saved_FA_pred_sol, title = "\nWT", xlabel = "", idxs=2, linecolor="red", linestyle=:dash, titlefontsize=10)
	gef_plot_KO = plot(saved_FA_KO_pred_sol, title = "DLC1-KO", xlabel = "Time [s]", idxs=1, linecolor="blue", linestyle=:dot, legend_position=:topleft, titlefontsize=10)
	plot!(twinx(), saved_FA_KO_pred_sol, title = "", idxs=2, linecolor="blue", linestyle=:dash, xlabel = "")
...
ForwardDiff = "~0.10.36"
ModelingToolkit = "~9.2.0"
Optim = "~1.9.2"
Optimization = "~3.23.0"
OptimizationPolyalgorithms = "~0.2.0"
Plots = "~1.40.1"
SciMLSensitivity = "~7.56.0"
"""
...
manifest_format = "2.0"
project_hash = "446beb285a10eb00d88273bf37f52225054a7b18"
...
git-tree-sha1 = "512a6072b362bb24a0579ddb78dba3ea4dd551db"
...
git-tree-sha1 = "c04ad5c94dfc1b6ef8d88d314c63355e7a4b3507"
