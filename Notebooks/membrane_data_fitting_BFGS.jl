...
begin
	...
	f(t) = 2 * (160 <= t <= 160.1) + (160.1 < t) + (t < 160)
...
# Define RhoA negative feedback model
...
begin
	...
	parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 
		0.5, 0.5]
	RhoA_prob = ODEProblem(RhoA_model, 
		[RhoA_model.aGEF => 0.5, 
			RhoA_model.aRhoA => 1,
			RhoA_model.aGAP => 0.5], 
		(150.0, 400.0), parameter_guess[1:9], jac = true)
end
...
@mtkmodel RhoA_KO_dynamics begin
	...
	@parameters begin
		#Kinact_GEF = 0.0376 (value obtained from WT data fitting)
		#Kcat_aGEF = 3.666 (value obtained from WT data fitting)
	...
	@equations begin
		D(aGEF) ~ 0.014 * (GEF0 * S - aGEF)
		D(aGAP) ~ Kcat_FL * ((10 - aGAP) * (aRhoA-1)^2)/(0 + (10 - aGAP)) + Kinact_GAP * (GAP0 - aGAP)
		
		D(aRhoA) ~ 3.675 * (aGEF-GEF0) * (10 - aRhoA)/(10 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0)^2 * aRhoA/(0 + aRhoA)
		S ~ f_tanh(t)
...
function MSE_loss(new_parameters)
	...
	new_p = new_parameters[1:9]       # this performs a deep copy
...
begin
	...
	GEF0 = optim_results.u[10]
	GAP0 = optim_results.u[11]
	pred_u0 = [GEF0; GAP0; 1]

	# Re-modify parameters as for optimisation
	pred_p = optim_results[1:9]
...
begin
	...
	saved_parameters = [4.97716, 0, 10.0, 0.0293311, 3.66671, 0.0464938, 5.05468, 0.00842722, 0, 0.0376053, 0.837458, 0.896395] 
	...
	saved_param = [
		...
		RhoA_model.GEF0 => saved_parameters[11],
		RhoA_model.GAP0 => saved_parameters[12]
	]
	saved_u0 = [saved_parameters[11:12]; 1]
...
function KO_MSE_loss(new_parameters)
	...
	# Substitute the initial conditions in the parameters
	new_u0 = [new_parameters[6:7]; [1]]
	new_p = new_parameters[1:5]
	new_p[get_idxparam(RhoA_KO_model.GEF0, RhoA_KO_prob)] = new_parameters[6]
	new_p[get_idxparam(RhoA_KO_model.GAP0, RhoA_KO_prob)] = new_parameters[7] 
...
begin
	...
	# Define upper and lower bound for parameters and initial conditions
	KO_lower_b = repeat([0], 7)
	KO_upper_b = [repeat([10], 5); [2, 2]]
...
begin
	# Retrieve initial conditions from optimisation solution
	GEF0_KO = KO_optim_results.u[6]
	GAP0_KO = KO_optim_results.u[7]
	KO_pred_u0 = [GEF0_KO; GAP0_KO; [1]]

	# Re-modify parameters as for optimisation
	KO_pred_p = KO_optim_results[1:5]
...
begin
	# Save best parameters
	saved_KO_parameters = [0.0148107, 5.00002, 3.3423, 0.0402467, 5.0, 1.10217, 1]
	...
	saved_KO_param = [
		...
		RhoA_KO_model.GEF0 => saved_KO_parameters[6],
		RhoA_KO_model.GAP0 => saved_KO_parameters[7]]
	saved_KO_u0 = [saved_KO_parameters[6:7]; 1]
