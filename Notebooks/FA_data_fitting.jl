...
# ╔═╡ 51cded62-b8f2-4077-9670-db41af221550
using SymbolicIndexingInterface

# ╔═╡ 837eccab-c884-4e27-b6c3-d6f97b9791bb
...
begin
	...
	f_tanh(t) = 2 * (tanh(100*(t-159))/2 - tanh(100*(t-162))/2) 
...
# the step function
begin
	x = LinRange(0, 400, 400)
	y1 = f.(x)
	y2 = f_tanh.(x)
	plot(x, [y1, y2], labels =["Step function" "Tanh"], lw = 2, title="Comparison of light signal functions")
end

# ╔═╡ 298dde08-6fc8-463f-91bc-fabfd8c78a9e
md"""
#### 1: Fit RhoA negative feedback model to FA NON-KO data:
"""
...
begin
	...
	rhoa_data_FA = median_rhoa_FA[16:length(median_rhoa_FA)]
	rhoa_data_FA_KO = median_rhoa_FA_KO[16:length(median_rhoa_FA_KO)]
end
...
@mtkmodel RhoA_dynamics begin
	...
	@equations begin
		D(aGEF) ~ Kact_light * S - Kinact_GEF * aGEF + Kact_GEF
		D(aGAP) ~ Kcat_FL * ((10 - aGAP) * aRhoA)/(Km_FL + (10 - aGAP)) + Kact_GAP - Kinact_GAP * aGAP
		D(aRhoA) ~  Kcat_aGEF * aGEF * (10 - aRhoA)/(Km_aGEF + (10 - aRhoA)) - (Kcat_aGAP * (aGAP) * aRhoA/(0.1 + aRhoA))
		S ~ f_tanh(t)
	end
end
...
begin
	...
	parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.2, 0.51, 0.52]
	
	RhoA_prob = ODEProblem(RhoA_model, 
		[RhoA_model.aGEF => 0.5, 
			RhoA_model.aRhoA => 1,
			RhoA_model.aGAP => 1], 
		(160.0, 400.0), parameter_guess[1:10], jac = true)
end

# ╔═╡ acfa8a3b-e215-4332-8389-ed3356feec51
RhoA_prob.p
...
function MSE_loss(new_parameters)
	...
	new_u0 = [new_parameters[11:12];[1]]
	new_p = new_parameters[1:10]
	
	# Update the ODE problem with the new set of parameters
	new_ODE_prob = remake(RhoA_prob, p=new_p, u0=new_u0)

	# Solve the new ODE problem (saveat = numerical estimate time interval)
	new_sol = solve(new_ODE_prob, AutoVern9(Rodas4P()), saveat = 10, maxiters = 1e7, dtmax = 0.1, verbose = false)
	new_sol_array = new_sol[RhoA_model.aRhoA]

	# Determine the loss by computing the sum of squares
	if length(new_sol_array) == length(rhoa_data_FA)
		loss = sum(abs2, new_sol_array .- rhoa_data_FA) + abs(new_sol[RhoA_model.aGEF][1] - new_sol[RhoA_model.aGEF][length(new_sol_array)]) + abs(new_sol[RhoA_model.aGAP][1] - new_sol[RhoA_model.aGAP][length(new_sol_array)])
		#
	else
		loss = 1000
	end
...
callback! = function(parameters, loss, new_sol)
...
begin 
	...
	optim_results = Optimization.solve(optimisation_prob, PolyOpt(), callback = callback!, maxiters = 100, progress = true)
end
  ╠═╡ =#
...
#=╠═╡
begin
	predicted_parameters = ModelingToolkit.MTKParameters(RhoA_model, optim_results.u[1:10])
	predicted_u0 = [optim_results.u[11:12]; 1]
	pred_sol = solve(remake(RhoA_prob, p=optim_results.u[1:10], u0=predicted_u0), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_data_FA, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
	plot!(pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
end
  ╠═╡ =#

# ╔═╡ 938add57-23e4-432e-841e-6bbcaa9d859c
begin
	saved_parameters = [0.0128385, 0.151514, 0.110561, 0.0433963, 0.0734581, 0.056111, 0.0329915, 0.0169319, 0.0113834, 0.0179421, 0.716493, 1.02211]
	mtk_saved_param = ModelingToolkit.MTKParameters(RhoA_model, saved_parameters[1:10])
	
	saved_pred_sol = solve(remake(RhoA_prob, p=saved_parameters[1:10], u0=[saved_parameters[11:12]; 1]), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_data_FA, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
	plot!(saved_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
	
end

# ╔═╡ 09048715-8b24-4a93-b935-3d4e9574d069
plot(saved_pred_sol, title = "aGEF, aGAP and aRhoA simultaneous dynamics", xlabel = "Time [s]")
...
md"""
#### 2: Fit RhoA negative feedback model to DLC1-KO data:
"""

# ╔═╡ 576f576c-1aa3-4d79-924c-0af1dbd11753
begin
	getKactLight = getp(RhoA_model, RhoA_model.Kact_light)
	getKcatGEF = getp(RhoA_model, RhoA_model.Kcat_aGEF)
	getKmGEF = getp(RhoA_model, RhoA_model.Km_aGEF)
end

# ╔═╡ a813ba78-cdeb-49d1-8780-9030b64104a7
getKactLight(RhoA_prob.p)

# ╔═╡ b75ecb7f-e7d5-4201-a360-c6982ec909a2
#=╠═╡
getKactLight(predicted_parameters)
  ╠═╡ =#

# ╔═╡ cd54d525-143f-4339-a689-b22ad2b826a4
#=╠═╡
getKcatGEF(predicted_parameters)
  ╠═╡ =#

# ╔═╡ 37309972-1ab4-4e74-b9eb-2556b645b4fa
#=╠═╡
getKmGEF(predicted_parameters)
  ╠═╡ =#
...
@mtkmodel RhoA_KO_dynamics begin
	...
	@equations begin
		D(aGEF) ~ 0.0169319 * S - Kinact_GEF * aGEF + Kact_GEF
		D(aGAP) ~ Kcat_FL * ((10 - aGAP) * aRhoA^3)/(Km_FL + (10 - aGAP)) - Kinact_GAP * aGAP + Kact_GAP
		D(aRhoA) ~ 0.0734581 * aGEF * (10 - aRhoA)/(0.110561 + (10 - aRhoA)) - (Kcat_aGAP * aGAP * aRhoA/(Km_aGAP + aRhoA))
		S ~ f_tanh(t)
...
begin
	...
	# Define some parameters for the FL model
	KO_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.5, 0.5]  # Last two are initial conditions for aGEF and aGAP
	
	RhoA_KO_prob = ODEProblem(RhoA_KO_model, 
		[RhoA_KO_model.aGEF => 0.5, 
			RhoA_KO_model.aRhoA => 1, 
			RhoA_KO_model.aGAP => 1], 
		(160.0, 400.0), KO_parameter_guess[1:8], jac = true)
end
...
function KO_MSE_loss(new_parameters)
	...
	new_u0 = [new_parameters[9:10]; [1]]
	new_p = new_parameters[1:8] 
	
	# Update the ODE problem with the new set of parameters
	KO_new_ODE_prob = remake(RhoA_KO_prob, u0=new_u0, p=new_p)

	# Solve the new ODE problem (saveat = numerical estimate time interval)
	KO_new_sol = solve(KO_new_ODE_prob, Rodas4(), saveat = 10, maxiters = 1e7, dtmax = 0.5)
	KO_new_sol_array = KO_new_sol[RhoA_KO_model.aRhoA]

	# Determine the loss by computing the sum of squares¨
	if length(KO_new_sol_array) == length(rhoa_data_FA_KO)
		loss = sum(abs2, KO_new_sol_array .- rhoa_data_FA_KO) + (abs2(KO_new_sol[RhoA_KO_model.aGEF][1] - KO_new_sol[RhoA_KO_model.aGEF][length(KO_new_sol_array)]) + abs2(KO_new_sol[RhoA_KO_model.aGAP][1] - KO_new_sol[RhoA_KO_model.aGAP][length(KO_new_sol_array)]))

	else
		loss = 1000
	end

	return loss, KO_new_sol
end
...
begin
	...
	KO_optimisation_prob = Optimization.OptimizationProblem(KO_optimisation_f, KO_parameter_guess)
end

# ╔═╡ 764df649-41df-417e-bfc9-19376333550d
# Define callback function that is run at each iteration
KO_callback! = function(parameters, loss, new_sol)
...
begin 
	...
	# Run the optimisation
	KO_optim_results = Optimization.solve(KO_optimisation_prob, PolyOpt(), callback = KO_callback!, maxiters = 100, progress = true)
end
  ╠═╡ =#
...
#=╠═╡
begin
	KO_predicted_parameters = KO_optim_results.u[1:8]
	KO_predicted_u0 = [KO_optim_results.u[9:10]; 1] 
	KO_pred_sol = solve(remake(RhoA_KO_prob, u0=KO_predicted_u0, p=KO_predicted_parameters), saveat=1, dtmax=0.1)
	scatter(timeframe, rhoa_data_FA_KO, label = "Data", lw=2, title = "Membrane RhoA dynamics in DLC1-KO cells")
	plot!(KO_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
end
  ╠═╡ =#

# ╔═╡ c41fcedc-40d1-4332-87d4-8d6a5549408b
RhoA_KO_prob.p

# ╔═╡ 259c7f4b-fe37-48ad-a5a9-640f460e1dc8
begin
	saved_KO_parameters = [0.00498952, -0.309675, 0.0527049, 0.0513012, 0.0321619, 0.0112495, 0.251191, 0.00613419, 0.827409, 0.814542]
	mtk_saved_KO_param = ModelingToolkit.MTKParameters(RhoA_KO_model, saved_KO_parameters[1:8])
	
	saved_KO_pred_sol = solve(remake(RhoA_KO_prob, p=saved_KO_parameters[1:8], u0=[saved_KO_parameters[9:10]; 1]), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_data_FA_KO, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
	plot!(saved_KO_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
	
end

# ╔═╡ b26cf811-d651-4fa3-9a67-34318c790548
plot(saved_KO_pred_sol, title = "aGEF, aGAP and aRhoA simultaneous dynamics", xlabel = "Time [s]")

# ╔═╡ 994791ee-6ca4-42cf-bdcb-d2d04ff85890
md"""
### Comparison between DLC1-KO and WT data fitting
"""

# ╔═╡ d0b56bca-b8af-4879-b376-79ed81a88f1a
begin
	plot(saved_KO_pred_sol, lw=2, idxs=3, linecolor="steelblue", label = "DLC1-KO aRhoA")
	scatter!(timeframe, rhoa_data_FA_KO, label = "DLC1-KO Data", lw=2, title = "Membrane RhoA dynamics", markercolor="blue")
	plot!(saved_pred_sol, lw=2, idxs=3, linecolor="red",label = "WT aRhoA")
	scatter!(timeframe, rhoa_data_FA, label = "WT Data", markercolor="red3")
	xlabel!("Time [s]")
