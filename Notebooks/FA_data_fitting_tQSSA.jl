...
using DifferentialEquations, Optimization, OptimizationPolyalgorithms, SciMLSensitivity, ModelingToolkit, Optim,OptimizationOptimisers

# ╔═╡ 7f53f00a-f877-4dbb-901d-572d0bd6c54a
using ForwardDiff, Plots

# ╔═╡ e73797f0-b512-446e-90a6-8962d6f49a14
...
begin
	...
	f_tanh(t) = 1 + (tanh(100*(t-159))/2 - tanh(100*(t-161))/2)
...
begin
	...
	timeframe = LinRange(160, 400, 25)
end

# ╔═╡ a7e00912-b724-4ad4-85c7-aa1d740591d8
# Adapt negative feedback model to experimental initial conditions and timeline
begin
...
# ╔═╡ 44fd54e8-77c0-4029-bd28-d1151ea6e260
# Save data 
begin
	rhoa_FA_KO_data = median_rhoa_FA_KO[16:length(median_rhoa_FA_KO)]
	rhoa_FA_data = median_rhoa_FA[16:length(median_rhoa_FA)]
end
...
@mtkmodel RhoA_FA_dynamics begin
	...
	@equations begin
		...
		D(aGAP) ~ Kcat_FL * 0.5 * (Km_FL + (aRhoA-1) + (10 - aGAP) - sqrt((Km_FL + (aRhoA-1) + (10 - aGAP))^2 - 4 * (aRhoA-1) * (10 - aGAP))) + Kinact_GAP * (GAP0 -aGAP)
		D(aRhoA) ~ Kcat_aGEF * 0.5 * (Km_aGEF + (aGEF-GEF0) + (10 - aRhoA) - sqrt((Km_aGEF + (aGEF-GEF0) + (10 - aRhoA))^2 - 4 * (aGEF-GEF0) * (10 - aRhoA))) - Kcat_aGAP  * 0.5 * (Km_aGAP + (aGAP-GAP0) + (aRhoA) - sqrt((Km_aGAP + (aGAP-GAP0) + (aRhoA))^2 - 4 * aRhoA * (aGAP-GAP0)))  
		S ~ f_tanh(t)
	end
end
...
begin
	...
	# Define some parameters for the RhoA FA model
	...
	RhoA_FA_prob = ODEProblem(RhoA_FA_model, 
		[RhoA_FA_model.aGEF => 0.65, 
			RhoA_FA_model.aRhoA => 1,
			RhoA_FA_model.aGAP => 0.73], 
		(160.0, 400.0), FA_parameter_guess[1:10], jac = true)
end

# ╔═╡ 92bb83f2-2b93-4327-93af-161044a9abc3
RhoA_FA_prob.p
...
function FA_MSE_loss(new_parameters)
	...
	new_aGEF0 = new_parameters[11]
	new_aGAP0 = new_parameters[12]
	new_u0 = [new_aGEF0; new_aGAP0; 1]
	new_p = [new_aGAP0; new_parameters[2:6]; new_aGEF0; new_parameters[8:10]]
	...
	FA_new_sol = solve(FA_new_ODE_prob, AutoVern9(Rodas4P()), saveat = 10, verbose = false, maxiters = 1e7, dtmax=0.1)
	FA_new_sol_array = FA_new_sol[RhoA_FA_model.aRhoA]

	# Determine the loss by computing the sum of squares
	if length(FA_new_sol_array) == length(rhoa_FA_data)
		loss = sum(abs2, FA_new_sol_array .- rhoa_FA_data)
	else
		loss = 1000
	end
...
begin
	...
	FA_optimisation_prob = Optimization.OptimizationProblem(FA_optimisation_f, FA_parameter_guess)
...
begin 
	...
	FA_optim_results = Optimization.solve(FA_optimisation_prob, PolyOpt(), callback = FA_callback!, maxiters = 100, progress = true)
end
...
begin
	...
	pred_FA_param = [GAP0; FA_optim_results.u[2:6]; GEF0; FA_optim_results.u[8:10]]
	FA_pred_sol = solve(remake(RhoA_FA_prob, p=pred_FA_param, u0=[GEF0; GAP0; 1], tspan=(160, 400)), saveat=0.5, dtmax = 0.1)
	#FA_pred_sol = solve(remake(RhoA_FA_prob, p=FA_optim_results.u), saveat=1, dtmax = 0.1)
	scatter(timeframe, rhoa_FA_data, label = "Data", lw=2, title = "FA RhoA dynamics in WT cells")
	plot!(FA_pred_sol, label = "Prediction", lw=2, idxs=3, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
end

# ╔═╡ 47bc1ec5-d18d-4846-bae7-487c6e23ef85
#=╠═╡
plot(FA_pred_sol, label = "Prediction", lw=2, idxs=2, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
  ╠═╡ =#
...
begin
	saved_FA_parameters = [0.1, 0.128564, 0.046817, 0.248333, 0.0106267, 0.16, 0.042064, 0.0248643, 0.0151197, 0.598715, 0.605839]

	saved_FA_p = [saved_FA_parameters[11]; saved_FA_parameters[2:6]; saved_FA_parameters[10]; saved_FA_parameters[8:10]]
	saved_FA_u0 = [saved_FA_parameters[10:11]; 1]
	
	saved_FA_pred_sol = solve(remake(RhoA_FA_prob, p=saved_FA_p, u0=saved_FA_u0), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_FA_data, label = "Data", lw=2, title = "FA RhoA dynamics in WT cells")
...
@mtkmodel RhoA_FA_KO_dynamics begin
	...
	@equations begin
		D(aGEF) ~ Kinact_GEF * (aGEF0 * S - aGEF) 
		D(aGAP) ~ Kcat_FL * ((aRhoA-1)^2 * (10 - aGAP))/(Km_FL + (10 - aGAP)) + Kinact_GAP * (aGAP0-aGAP)
		D(aRhoA) ~ 0.57397 * (aGEF-aGEF0) * (10 - aRhoA)/(0.00771422 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-aGAP0) #* (aRhoA)/(Km_aGAP + (aRhoA)))
		S ~ f_tanh(t)
...
begin
	...
	FA_KO_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.2, 0.21]  
	
	RhoA_FA_KO_prob = ODEProblem(RhoA_FA_KO_model, 
		[RhoA_FA_KO_model.aGEF => 0.5, 
			RhoA_FA_KO_model.aRhoA => 1, 
			RhoA_FA_KO_model.aGAP => 0.5], 
		(160.0, 400.0), FA_KO_parameter_guess[1:7], jac = true)
end

# ╔═╡ a378cf8e-bb5c-410f-b99e-72445d34264d
RhoA_FA_KO_prob.p
...
function FA_KO_MSE_loss(new_parameters)
	...
	new_p = [new_parameters[9];new_parameters[8]; new_parameters[3:7]] 
	
	# Update the ODE problem with the new set of parameters
	FA_KO_new_ODE_prob = remake(RhoA_FA_KO_prob, p=new_p, u0=new_u0)

	# Solve the new ODE problem (saveat = numerical estimate time interval)
	FA_KO_new_sol = solve(FA_KO_new_ODE_prob, AutoVern9(Rodas4P()), saveat = 10, maxiters = 1e7, verbose = false, dtmax=0.1)
	FA_KO_new_sol_array = FA_KO_new_sol[RhoA_FA_KO_model.aRhoA]

	# Determine the loss by computing the sum of squares¨
	if length(FA_KO_new_sol_array) == length(rhoa_FA_KO_data)
		FA_loss = sum(abs2, FA_KO_new_sol_array .- rhoa_FA_KO_data)
	else
		FA_loss = 1000
	end
...
begin
	...
	FA_KO_optimisation_prob = Optimization.OptimizationProblem(FA_KO_optimisation_f, FA_KO_parameter_guess)
end
...
begin 
	...
	# Run the optimisation
	FA_KO_optim_results = Optimization.solve(FA_KO_optimisation_prob, PolyOpt(), callback = FA_KO_callback!, maxiters = 100, progress = true)
end
  ╠═╡ =#
...
#=╠═╡
begin
	FA_KO_predicted_parameters = [FA_KO_optim_results.u[9]; FA_KO_optim_results.u[8]; FA_KO_optim_results.u[3:7]]
	FA_KO_predicted_u0 = [FA_KO_optim_results.u[8:9]; [1]]

	FA_KO_pred_sol = solve(remake(RhoA_FA_KO_prob, p=FA_KO_predicted_parameters, u0=FA_KO_predicted_u0), saveat=1, dtmax=0.1, tspan=(160, 400))
	scatter(timeframe, rhoa_FA_KO_data, label = "Data", lw=2, title = "RhoA dynamics in FA and DLC1-KO cells")
...
begin
	saved_FA_KO_parameters = [0.1, 0.12, 0.0204882, 0.0958295, 0.0636916, 0.274359, 0.0121646, 0.743502, 0.321343]
	
	saved_FA_KO_param = [saved_FA_KO_parameters[9]; saved_FA_KO_parameters[8]; saved_FA_KO_parameters[3:7]]
	saved_FA_KO_u0 = [saved_FA_KO_parameters[8]; saved_FA_KO_parameters[9]; 1]
	
	saved_FA_KO_pred_sol = solve(remake(RhoA_FA_KO_prob, p=saved_FA_KO_param, u0 = saved_FA_KO_u0), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_FA_KO_data, label = "Data", lw=2, title = "FA RhoA dynamics in WT cells")
...
begin
	...
	scatter!(timeframe, rhoa_FA_KO_data, label = "DLC1-KO Data", lw=2, title = "Membrane RhoA dynamics", markercolor="blue")
	plot!(saved_FA_pred_sol, lw=2, idxs=3, linecolor="red",label = "WT aRhoA")
	scatter!(timeframe, rhoa_FA_data, label = "WT Data", markercolor="red3")
	xlabel!("Time [s]")
...
manifest_format = "2.0"
project_hash = "b3c677f753dcd4dbfb0e040f02bc7db628333ccf"
