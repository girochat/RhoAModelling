...
begin
	...
	f(t) = 2 * (160 <= t <= 160.8) + (160.8 < t) + (t < 160)
...
# Visualise data
begin
	plot(median_rhoa_NONFA_KO, label = "RhoA DLC1-KO")
	plot!(median_rhoa_NONFA, label = "RhoA DLC1", title= "RhoA dynamics at membrane")
end
...
# Define RhoA negative autoregulation model for membrane dynamics
...
begin
	...
	parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.2, 0.21, 0.22, 0.5, 0.5]
	...
	RhoA_prob = ODEProblem(RhoA_model, 
		[RhoA_model.aGEF => 0.5, 
			RhoA_model.aRhoA => 1,
			RhoA_model.aGAP => 0.5], 
		(150.0, 400.0), parameter_guess[1:12], jac = true)
end
...
@mtkmodel RhoA_KO_dynamics begin
	...
	@equations begin
		D(aGEF) ~ 0.039 * (GEF0 * S - aGEF)
		D(aGAP) ~ Kcat_FL * ((10 - aGAP) * (aRhoA-1) * (tanh(100 * (aRhoA-1))-tanh(100*(aRhoA-10)))/2)/(Km_FL + (10 - aGAP)) + Kinact_GAP * (GAP0 - aGAP)
		D(aRhoA) ~ 0.614 * (aGEF-GEF0) * (10 - aRhoA)/(10 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0) * aRhoA/(Km_aGAP + aRhoA)
		S ~ f_tanh(t)
...
begin
	...
	# Define some initial parameters for the ODE model
	KO_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.5, 0.5]
	...
	RhoA_KO_prob = ODEProblem(RhoA_KO_model, 
		...
		(150.0, 400.0), KO_parameter_guess[1:7], jac = true)
end
...
begin
	...
	GEF0_KO = KO_optim_results.u[8]
	GAP0_KO = 2.5 #KO_optim_results.u[9]
	KO_pred_u0 = [GEF0_KO; GAP0_KO; [1]]

	# Re-modify parameters as for optimisation
	KO_pred_p = KO_optim_results[1:7]
	KO_pred_p[get_idxparam(RhoA_KO_model.GEF0, RhoA_KO_prob)] = GEF0_KO
	KO_pred_p[get_idxparam(RhoA_KO_model.GAP0, RhoA_KO_prob)] = GAP0_KO 

	# Solve and plot final solution
	KO_pred_sol = solve(remake(RhoA_KO_prob, p=KO_pred_p, u0=KO_pred_u0, tspan=(120, 400)), saveat=1, dtmax=0.01)
...
# ╔═╡ 09291e2c-27a8-4227-943a-39b0b68812ff
# ╠═╡ disabled = true
#=╠═╡
begin
	#plot(FA_KO_pred_sol, title = "aGEF, aGAP and aRhoA simultaneous dynamics", xlabel = "Time [s]", idxs=2)
	gap_plot = plot(saved_pred_sol, title = "", xlabel = "Time [s]", idxs=2, linecolor="red")
	gef_plot = plot(saved_pred_sol, title = "WT", xlabel = "Time [s]", idxs=1, linecolor="red")
	gef_plot_KO = plot(saved_KO_pred_sol, title = "DLC1-KO", xlabel = "Time [s]", idxs=1, linecolor="blue")
	gap_plot_KO = plot(saved_KO_pred_sol, title = "", xlabel = "Time [s]", idxs=2, linecolor="blue")
	plot(gef_plot, gef_plot_KO, gap_plot, gap_plot_KO, layout=(2, 2), legend=true)
	#savefig("../Data/membrane_all_plot.svg")
end
  ╠═╡ =#
...
function MSE_loss(new_parameters)
	...
	new_p[get_idxparam(RhoA_model.GEF0, RhoA_prob)] = new_GEF0
	new_p[get_idxparam(RhoA_model.GAP0, RhoA_prob)] = new_GAP0
	
	# Update the ODE problem with the new set of parameters
	new_ODE_prob = remake(RhoA_prob, p=new_p, u0=new_u0) #AutoVern9(Rodas4P())

	# Solve the new ODE problem (saveat = numerical estimate time interval)
	new_sol = solve(new_ODE_prob, Rosenbrock23(), saveat = 10, maxiters = 1e7, dtmax = 0.05, verbose = false)
...
begin 
	...
	# Run the optimisation
	optim_results = Optimization.solve(optimisation_prob, BFGS(s), callback = callback!, maxiters = 100, progress = true)
end
...
begin
	# Retrieve initial conditions from optimisation solution
	GEF0 = optim_results.u[13]
	GAP0 = optim_results.u[14]
	pred_u0 = [GEF0; GAP0; 1]

	# Re-modify parameters as for optimisation
	pred_p = optim_results[1:12]
	pred_p[get_idxparam(RhoA_model.GEF0, RhoA_prob)] = GEF0
	pred_p[get_idxparam(RhoA_model.GAP0, RhoA_prob)] = GAP0

	# Solve the ODE problem with optimal parameters
	pred_sol = solve(remake(RhoA_prob, p=pred_p, u0=pred_u0, tspan=(120, 400)), saveat=1, dtmax=0.05)
...
begin
	...
	saved_parameters = [0.145289, 7.82794, 10.0, 0, 0.391736, 0.137523, 0.108948, 0, 0.209846, 8.21694, 0, 0, 0.22743, 9.99904] 
	
	# Transform back ODE parameters to their symbolic expression
	saved_param = [
		RhoA_model.Kinact_GEF => saved_parameters[get_idxparam(RhoA_model.Kinact_GEF, RhoA_prob)],
		RhoA_model.Kcat_aGEF => saved_parameters[get_idxparam(RhoA_model.Kcat_aGEF, RhoA_prob)], 
		RhoA_model.Km_aGEF => saved_parameters[get_idxparam(RhoA_model.Km_aGEF, RhoA_prob)],
		RhoA_model.Kinact_GAP => saved_parameters[get_idxparam(RhoA_model.Kinact_GAP, RhoA_prob)],
		RhoA_model.Kcat_FL => saved_parameters[get_idxparam(RhoA_model.Kcat_FL, RhoA_prob)],
		RhoA_model.Km_FL => saved_parameters[get_idxparam(RhoA_model.Km_FL, RhoA_prob)],
		RhoA_model.Kcat_aGAP => saved_parameters[get_idxparam(RhoA_model.Kcat_aGAP, RhoA_prob)],
		RhoA_model.Km_aGAP => saved_parameters[get_idxparam(RhoA_model.Km_aGAP, RhoA_prob)],
		RhoA_model.GEF0 => saved_parameters[9],
		RhoA_model.GAP0 => saved_parameters[10]
	]
	saved_u0 = [saved_parameters[9:10]; 1]

	# Solve the ODE with the best parameters and plot
	saved_pred_sol = solve(remake(RhoA_prob, p=saved_param, u0=saved_u0, tspan=(120, 400)), saveat=1, dtmax=0.05) 
	scatter(timeframe[12:end], median_rhoa_NONFA[12:end], label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
...
function KO_MSE_loss(new_parameters)
	...
	new_p[get_idxparam(RhoA_KO_model.GEF0, RhoA_KO_prob)] = new_parameters[8]
	new_p[get_idxparam(RhoA_KO_model.GAP0, RhoA_KO_prob)] = new_parameters[9]
	
	# Update the ODE problem with the new set of parameters
	KO_new_ODE_prob = remake(RhoA_KO_prob, p=new_p, u0 = new_u0)
...
begin
	...
	# Define upper and lower bound for parameters and initial conditions
	KO_lower_b = repeat([0], 9)
	KO_upper_b = [repeat([10], 7); [10, 10]]
...
begin
	# Save best parameters
	saved_KO_parameters = [0, 0.0146779, 5.00001, 5.0222, 0.0325226, 0, 5.00002, 0.815275, 2.49404] # [0, 0.0146779, 4.99999, 5.24192, 0.0318337, 0, 4.99999, 0.370799, 0.647862] #[0, 0.0146929, 5.0, 5.01208, 0.0326741, 0, 5.0, 1.19936, 0.891153]

	# Transform back ODE parameters to their symbolic expression
	saved_KO_param = [
		RhoA_KO_model.Kinact_GAP => saved_KO_parameters[get_idxparam(RhoA_KO_model.Kinact_GAP, RhoA_KO_prob)],
		RhoA_KO_model.Kcat_FL => saved_KO_parameters[get_idxparam(RhoA_KO_model.Kcat_FL, RhoA_KO_prob)], 
		RhoA_KO_model.Km_FL => saved_KO_parameters[get_idxparam(RhoA_KO_model.Km_FL, RhoA_KO_prob)],
		RhoA_KO_model.Kcat_aGAP => saved_KO_parameters[get_idxparam(RhoA_KO_model.Kcat_aGAP, RhoA_KO_prob)],
		RhoA_KO_model.Km_aGAP => saved_KO_parameters[get_idxparam(RhoA_KO_model.Km_aGAP, RhoA_KO_prob)],
		RhoA_KO_model.GEF0 => saved_KO_parameters[8],
		RhoA_KO_model.GAP0 => saved_KO_parameters[9]]
	saved_KO_u0 = [saved_KO_parameters[8:9]; 1]

	# Solve the ODE with the best parameters and plot
	saved_KO_pred_sol = solve(remake(RhoA_KO_prob, p=saved_KO_param, u0=saved_KO_u0, tspan=(120, 400)), saveat=1, dtmax=0.05) 
	scatter(timeframe[12:end], median_rhoa_NONFA_KO[12:end], label = "Data", lw=2, title = "Membrane RhoA dynamics in DLC1-KO cells")
...
begin
	...
	scatter!(timeframe[12:end], median_rhoa_NONFA_KO[12:40], label = "DLC1-KO (Data)", lw=2, title = "Membrane RhoA dynamics", markercolor="blue")
	plot!(saved_pred_sol, lw=2, idxs=3, linecolor="red",label = "WT (Prediction)")
	scatter!(timeframe[12:end], median_rhoa_NONFA[12:end], label = "WT (Data)", markercolor="red3")
	xlabel!("Time [s]")
	ylabel!("Fold-change \nnormalised to baseline")
	#savefig("../Data/membrane_plot.svg")
end
