...
using DifferentialEquations, Optimization, OptimizationPolyalgorithms, SciMLSensitivity, ModelingToolkit, Optim
...
@mtkmodel RhoA_FA_dynamics begin
	...
	@parameters begin
		#Kinact_GEF = 0.034 (iLID deactivation rate)
		Kcat_aGEF
		#Km_aGEF = 5
...
begin
	...
	FA_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.5, 0.5] 
	...
	RhoA_FA_prob = ODEProblem(RhoA_FA_model, 
		...
		(150.0, 400.0), 
		FA_parameter_guess[1:8], jac = true)
end
...
@mtkmodel RhoA_FA_KO_dynamics begin
	...
	@equations begin
		...
		D(aRhoA) ~ 1.15 * (aGEF-GEF0) * (10 - aRhoA)/(5 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0)^2 * (aRhoA)/(Km_aGAP + (aRhoA))
		S ~ f_tanh(t)
...
function FA_MSE_loss(new_parameters)
	...
	new_GEF0 = new_parameters[9]
	new_GAP0 = new_parameters[10]
	new_u0 = [new_GEF0; new_GAP0; 1]
	new_p = new_parameters[1:8]       # (it performs a deep copy)
	...
	FA_new_sol = solve(FA_new_ODE_prob, AutoVern9(Rodas4P()), saveat = 10, verbose = false, maxiters = 1e7, dtmax=0.05)
	FA_new_sol_array = FA_new_sol[RhoA_FA_model.aRhoA]

	# Determine the loss by computing the sum of squares
	if length(FA_new_sol_array) == length(median_rhoa_FA[15:end])
...
begin
	...
	FA_lower_b = repeat([0], 10)
	FA_upper_b = [repeat([10], 8); 2; 2]
...
begin 
	...
	FA_optim_results = Optimization.solve(FA_optimisation_prob, LBFGS(), callback = FA_callback!, maxiters = 100, progress = true)
end
...
begin
	...
	GEF0 = FA_optim_results.u[9]
	GAP0 = FA_optim_results.u[10]
	FA_pred_u0 = [GEF0; GAP0; 1]

	# Re-modify parameters as for optimisation
	FA_pred_p = FA_optim_results[1:8]
...
plot(FA_pred_sol, title = "aGEF, aGAP and aRhoA simultaneous dynamics", xlabel = "Time [s]")#, idxs=1)

# ╔═╡ ceea52cf-7b52-44a1-af60-127c0eb76bde
begin
	# Save best parameters
	saved_FA_parameters = [5.00301, 0.0283273, 5.00519, 1.15435, 0.209104, 0.00149207, 0, 5.0079, 0.274516, 0.891834] # [5.02782, 0, 5.05636, 0.0460356, 1.87614, 0.0238225, 4.87295, 0.0367562, 0, 0.0150597, 0.304125, 0.896692]
	...
	saved_FA_param = [
		...
		RhoA_FA_model.GEF0 => saved_FA_parameters[9],
		RhoA_FA_model.GAP0 => saved_FA_parameters[10]
	]
	saved_FA_u0 = [saved_FA_parameters[9:10]; 1]
...
function FA_KO_MSE_loss(new_parameters)
	...
	# Substitute the initial conditions in the parameters
	GEF0_KO = new_parameters[8]
	GAP0_KO = new_parameters[9]
	new_u0 = [GEF0_KO; GAP0_KO ; [1]]
	new_p = new_parameters[1:7]
	new_p[get_idxparam(RhoA_FA_KO_model.GEF0, RhoA_FA_KO_prob)] = GEF0_KO 
	new_p[get_idxparam(RhoA_FA_KO_model.GAP0, RhoA_FA_KO_prob)] = GAP0_KO
...
begin
	# Save best parameters
	saved_FA_KO_parameters = [0, 0.014498, 5.00106, 5.20919, 0.027982, 0, 4.99982, 0.331008, 0.892072] #[0, 0.0145182, 5.00095, 5.01555, 0.0285428, 0, 5.00005, 0.198374, 0.891104] #[0, 0.0221964, 4.99306, 2.5004, 0.00226845, 0, 4.98228, 0.380376, 0.890964]
...
begin
	gef_plot = plot(saved_FA_pred_sol, title = "", xlabel = "", idxs=1, linecolor="red", linestyle=:dot, legend_font_pointsize=6, ylabel="relative [aGEF]", legend=:best)
	...
	gef_plot_KO = plot(saved_FA_KO_pred_sol, title = "DLC1-KO", xlabel = "Time [s]", idxs=1, linecolor="blue", linestyle=:dot, titlefontsize=10, legend_font_pointsize=6, ylabel="relative [aGEF]")
	plot!(timeframe[12:end], NaN.*timeframe[12:end], label = "aGAP", linestyle=:dash, linecolor="blue")
	plot!(twinx(), saved_FA_KO_pred_sol, title = "", idxs=2, linecolor="blue", linestyle=:dash, xlabel = "", legend=false, ylabel="relative [aGAP]")
...
# ╔═╡ dd533088-f853-407c-9666-f3c84b67d3f6
begin

	# Visualise the effect of exponent on concentration dependence
	kcat = 0.001
	aRhoA = LinRange(0, 0.2, 100)
	h(x) = kcat * x^2
	g(x) = kcat * x
	plot(aRhoA, h.(aRhoA))
	plot!(aRhoA, g.(aRhoA))
...
manifest_format = "2.0"
project_hash = "1a802e13247daeb7ea60f23c5d819718eb6c6824"
