...
begin
	...
	f(t) = 2 * (160 <= t <= 160.1) + (t < 160) + (160.1 < t)
	...
	f_tanh(t) = 1 + (tanh(100*(t-160))/2 - tanh(100*(t-160.1))/2)
...
@mtkmodel RhoA_FA_dynamics begin
	...
	@equations begin
		D(aGEF) ~ Kinact_GEF * (GEF0 * S - aGEF)
		D(aGAP) ~ Kcat_FL * ((aRhoA-1) * (10 - aGAP))/(Km_FL + (10 - aGAP)) + Kinact_GAP * (GAP0-aGAP)
...
begin
	...
	FA_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.2, 0.5, 0.5]
	...
	RhoA_FA_prob = ODEProblem(RhoA_FA_model, 
		...
		(150.0, 400.0), 
		FA_parameter_guess[1:10], jac = true)
end
...
@mtkmodel RhoA_FA_KO_dynamics begin
	...
	@equations begin
		D(aGEF) ~ 0.014 * (GEF0 * S - aGEF) 
		D(aGAP) ~ Kcat_FL * ((aRhoA-1)^2 * (10 - aGAP))/(Km_FL + (10 - aGAP)) + Kinact_GAP * (GAP0-aGAP)
		D(aRhoA) ~ 5.4 * (aGEF-GEF0) * (10 - aRhoA)/(10 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0) * (aRhoA)/(Km_aGAP + (aRhoA))
		S ~ f_tanh(t)
...
begin
	...
	# Define some parameters for the FL model
...
function FA_MSE_loss(new_parameters)
	...
	new_GEF0 = new_parameters[11]
	new_GAP0 = new_parameters[12]
	new_u0 = [new_GEF0; new_GAP0; 1]
	new_p = new_parameters[1:10]       # this performs a deep copy
...
begin
	...
	FA_lower_b = repeat([0], 12)
	FA_upper_b = [repeat([10], 10); 2; 2]
...
begin
	...
	GEF0 = FA_optim_results.u[11]
	GAP0 = FA_optim_results.u[12]
	FA_pred_u0 = [GEF0; GAP0; 1]

	# Re-modify parameters as for optimisation
	FA_pred_p = FA_optim_results[1:10]
...
plot(FA_pred_sol, title = "aGEF, aGAP and aRhoA simultaneous dynamics", xlabel = "Time [s]")

# ╔═╡ ceea52cf-7b52-44a1-af60-127c0eb76bde
begin
	# Save best parameters
	saved_FA_parameters = [6.13608, 0.139088, 10.0, 0.0436621, 5.39707, 1.19702, 3.80869, 0.000812414, 0, 0.0140998, 1.2001, 0.890054] 
	...
	saved_FA_param = [
		...
		RhoA_FA_model.GEF0 => saved_FA_parameters[11],
		RhoA_FA_model.GAP0 => saved_FA_parameters[12]
	]
	saved_FA_u0 = [saved_FA_parameters[11:12]; 1]
...
# ╔═╡ 91553659-b28a-4b96-b2fb-81d78a8735bf
plot(FA_KO_pred_sol, label = "Prediction", lw=2, idxs=2)

# ╔═╡ f5877b34-cefe-4653-acd1-7ae9344e7d4f
begin
	# Save best parameters
	saved_FA_KO_parameters = [0, 0.021535, 5.01513, 1.15381, 0.00494683, 0, 5.0152, 1.4974, 0.88881] #[0, 0.021535, 5.0, 0.0755498, 0.075549, 5.0, 1.49821, 1.0]
	...
	# Solve the ODE with the best parameters and plot
	saved_FA_KO_pred_sol = solve(remake(RhoA_FA_KO_prob, p=saved_FA_KO_param, u0 = saved_FA_KO_u0), saveat=1, dtmax=0.05) 
...
begin
	plot(saved_FA_KO_pred_sol, lw=2, idxs=3, linecolor="steelblue", label = "DLC1-KO aRhoA")
	scatter!(timeframe[12:end], median_rhoa_FA_KO[12:end], label = "DLC1-KO Data", lw=2, title = "Membrane RhoA dynamics", markercolor="blue")
	plot!(saved_FA_pred_sol, lw=2, idxs=3, linecolor="red",label = "WT aRhoA")
