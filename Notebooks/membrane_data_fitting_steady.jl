...
begin
	...
	rhoa_NONFA_data = median_rhoa_NONFA[16:length(median_rhoa_NONFA)]
	rhoa_NONFA_KO_data = median_rhoa_NONFA_KO[16:length(median_rhoa_NONFA_KO)]
end
...
@mtkmodel RhoA_dynamics begin
	...
	@equations begin
		D(aGEF) ~ 0.015 * (aGEF0 * S - aGEF)
		D(aGAP) ~ Kcat_FL * (aRhoA - 1) * (10 - aGAP)/(Km_FL + (10 - aGAP)) + Kinact_GAP * (aGAP0 - aGAP)
		D(aRhoA) ~  Kcat_aGEF * (aGEF - aGEF0) * (10 - aRhoA)/(Km_aGEF + (10 - aRhoA)) - Kcat_aGAP * (aGAP - aGAP0) # * (aRhoA)/(Km_aGAP + (aRhoA)))
		S ~ f_tanh(t)
...
begin
	...
	RhoA_prob = ODEProblem(RhoA_model, 
		[RhoA_model.aGEF => 0.65, 
			RhoA_model.aRhoA => 1,
			RhoA_model.aGAP => 0.72], 
		(160.0, 400.0), parameter_guess[1:8], jac = true)
end

# ╔═╡ 3fae1a20-4231-4758-a403-11e929d36a10
# Define MSE loss function to estimate parameters for membrane and WT RhoA data
function MSE_loss(new_parameters)

	new_u0 = [new_parameters[9:10]; [1]]
	new_p = [new_parameters[10]; new_parameters[9]; new_parameters[3:8]] 
	...
	if length(new_sol_array) == length(rhoa_NONFA_data)
		loss = sum(abs2, new_sol_array .- rhoa_NONFA_data)
	else
		loss = 1000
	end
...
# ╔═╡ 873c1035-8999-4605-9ccc-641f1ce244ed
RhoA_prob.p
...
begin
	pred_param = [optim_results.u[10]; optim_results.u[9]; optim_results.u[3:8]]
	pred_u0 = [optim_results.u[9:10]; 1]
	pred_sol = solve(remake(RhoA_prob, p=pred_param, u0=pred_u0), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_NONFA_data, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
...
#=╠═╡
begin
	saved_parameters = [0.0426966, 0.0432165, 0.0277072, 0.13998, 0.0135905, 0.0283401, 0.299771, 0.0419222]
	saved_pred_sol = solve(remake(RhoA_prob, p=saved_parameters), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_NONFA_data, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
...
@mtkmodel RhoA_KO_dynamics begin
	...
	@parameters begin
		#Kinact_GEF 	# = 0.0396828
		#Kcat_aGEF # =  0.560605, 0.0920402
		#Km_aGEF  # = 1.39041, 0.240696
	...
	@equations begin
		D(aGEF) ~ 0.015 * (aGEF0 * S - aGEF)
		D(aGAP) ~ Kcat_FL * ((5 - aGAP) * (aRhoA-1)^3)/(Km_FL + (5 - aGAP)) + Kinact_GAP * (aGAP0-aGAP)
		
		D(aRhoA) ~ 0.0920402 * (aGEF-aGEF0) * (5 - aRhoA)/(0.240696 + (5 - aRhoA)) -Kcat_aGAP * (aGAP-aGAP0) #* aRhoA/(Km_aGAP + aRhoA))
		S ~ f_tanh(t)
...
begin
	...
	RhoA_KO_prob = ODEProblem(RhoA_KO_model, 
		...
		(160.0, 400.0), KO_parameter_guess[1:6], jac = true)
end

# ╔═╡ 8db59829-752c-49cd-8b64-f933ca9c8803
RhoA_KO_prob.p
...
function KO_MSE_loss(new_parameters)
	...
	new_u0 = [new_parameters[7:8]; [1]]
	new_p = [new_parameters[8]; new_parameters[7]; new_parameters[3:6]] 
	
	# Update the ODE problem with the new set of parameters
	KO_new_ODE_prob = remake(RhoA_KO_prob, p=new_p, u0 = new_u0)

	# Solve the new ODE problem (saveat = numerical estimate time interval)
	KO_new_sol = solve(KO_new_ODE_prob, AutoVern9(Rodas4P()), saveat = 10, maxiters = 1e7, dtmax = 0.1, verbose = false)
	KO_new_sol_array = KO_new_sol[RhoA_KO_model.aRhoA]

	# Determine the loss by computing the sum of squares¨
	if length(KO_new_sol_array) == length(rhoa_NONFA_KO_data)
		loss = sum(abs2, KO_new_sol_array .- rhoa_NONFA_KO_data)
...
begin
	KO_pred_param = [KO_optim_results[8]; KO_optim_results[7]; KO_optim_results[3:6]]
	KO_pred_u0 = [KO_optim_results[7:8]; [1]]
	KO_pred_sol = solve(remake(RhoA_KO_prob, p=KO_pred_param, u0=KO_pred_u0), saveat=1, dtmax=0.1)
	scatter(timeframe, rhoa_NONFA_KO_data, label = "Data", lw=2, title = "Membrane RhoA dynamics in DLC1-KO cells" )
	plot!(KO_pred_sol, label = "Prediction", lw=2, idxs=2, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")
end

# ╔═╡ 9b78f787-2241-456d-81bc-b0b0d5a4061d
plot(KO_pred_sol, label = "Prediction", lw=2, idxs=2, ylabel = "Fold-change \nnormalised to baseline", xlabel = "Time [s]")

# ╔═╡ d758e312-70d8-41e3-a1dc-cc6ce8a744db
begin
	saved_KO_parameters = [0.1, -0.59828, 0.13, 0.0235086, 0.0659441, 0.0554104, 0.242438, 0.0124108, 0.915873, 0.591407]
	saved_KO_pred_sol = solve(remake(RhoA_KO_prob, p=saved_KO_parameters), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_NONFA_KO_data, label = "Data", lw=2, title = "Membrane RhoA dynamics in WT cells")
...
begin
	...
	scatter!(timeframe, rhoa_NONFA_KO_data, label = "DLC1-KO Data", lw=2, title = "Membrane RhoA dynamics", markercolor="blue")
	plot!(saved_pred_sol, lw=2, idxs=3, linecolor="red",label = "WT aRhoA")
	scatter!(timeframe, rhoa_NONFA_data, label = "WT Data", markercolor="red3")
	xlabel!("Time [s]")
...
manifest_format = "2.0"
project_hash = "9946906a6d6466a30abafa469667682d912621a8"
