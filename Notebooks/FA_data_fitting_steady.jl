...
@mtkmodel RhoA_FA_dynamics begin
	...
	@equations begin
		...
		D(aGAP) ~ Kcat_FL * ((aRhoA-1) * (5 - aGAP))/(Km_FL + (5 - aGAP)) + Kinact_GAP * (GAP0 - aGAP)
		D(aRhoA) ~ Kcat_aGEF * (aGEF-GEF0) * (5 - aRhoA)/(Km_aGEF + (5 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0) #* aRhoA/(Km_aGAP + aRhoA) 
		S ~ f_tanh(t)
	end
end
...
begin
	...
	RhoA_FA_prob = ODEProblem(RhoA_FA_model, 
		...
		(160.0, 400.0), FA_parameter_guess[1:9], jac = true)
...
function FA_MSE_loss(new_parameters)
	...
	new_aGEF0 = new_parameters[10]
	new_aGAP0 = new_parameters[11]
	new_u0 = [new_aGEF0; new_aGAP0; 1]
	new_p = [new_aGAP0; new_parameters[2:5]; new_aGEF0; new_parameters[7:9]]
...
begin
	...
	pred_FA_param = [GAP0; FA_optim_results.u[2:5]; GEF0; FA_optim_results.u[7:9]]
...
begin
	saved_FA_parameters = [0.124105, 0.00771422, 0.0464796, 0.57397, 0.0171098, 0.0585459, 0.341227, 0.0155297] 
	saved_FA_pred_sol = solve(remake(RhoA_FA_prob, p=saved_FA_parameters), saveat=1, dtmax=0.1) 
	scatter(timeframe, rhoa_FA_data, label = "Data", lw=2, title = "FA RhoA dynamics in WT cells")
