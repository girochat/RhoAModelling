...
@mtkmodel RhoA_FA_dynamics begin
	...
	@equations begin
		...
		D(aRhoA) ~ Kcat_aGEF * (aGEF-GEF0) * (10 - aRhoA)/(5 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0)
		S ~ f_tanh(t)=#
		
		D(aGEF) ~ 0.034 * (GEF0 * S - aGEF)
		D(aGAP) ~ Kcat_FL * (aRhoA-1) * (tanh(100*(aRhoA-1))-tanh(100*(aRhoA-1000)))/2 * (1000 - aGAP)/(Km_FL + (1000 - aGAP)) + Kinact_GAP * (GAP0-aGAP)
		D(aRhoA) ~ Kcat_aGEF * (aGEF-GEF0) * (1000 - aRhoA)/(Km_aGEF + (1000 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0) * (aRhoA)#/(Km_aGAP + (aRhoA))
		S ~ f_tanh(t)
	end
end
...
begin
	...
	FA_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.5, 0.5] 
	...
	RhoA_FA_prob = ODEProblem(RhoA_FA_model, 
		...
		(150.0, 400.0), 
		FA_parameter_guess[1:9], jac = true)
end
...
@mtkmodel RhoA_FA_KO_dynamics begin
	...
	@equations begin
		...
		D(aRhoA) ~ 1.26 * (aGEF-GEF0) * (10 - aRhoA)/(5 + (10 - aRhoA)) - Kcat_aGAP * (aGAP-GAP0)^2
		S ~ f_tanh(t)
...
begin
	...
	# Define some initial parameters for the ODE model
	FA_KO_parameter_guess = [0.1, 0.12, 0.13, 0.14, 0.15, 0.5, 0.5]  
	...
	RhoA_FA_KO_prob = ODEProblem(RhoA_FA_KO_model, 
		...
		(150.0, 400.0), 
		FA_KO_parameter_guess[1:5], jac = true)
end
...
function FA_MSE_loss(new_parameters)
	...
	new_GEF0 = new_parameters[10]
	new_GAP0 = new_parameters[11]
	new_u0 = [new_GEF0; new_GAP0; 1]
	new_p = new_parameters[1:9]       # (it performs a deep copy)
...
begin
	...
	FA_lower_b = repeat([0], 11)
	FA_upper_b = [repeat([10], 9); 2; 2]
...
begin
	# Retrieve initial conditions from optimisation solution
	GEF0 = FA_optim_results.u[10]
	GAP0 = FA_optim_results.u[11]
	FA_pred_u0 = [GEF0; GAP0; 1]

	# Re-modify parameters as for optimisation
	FA_pred_p = FA_optim_results[1:9]
...
begin
	...
	saved_FA_parameters = [10.0, 0.0428104, 0.0260797, 0.973545, 0.036484, 4.99673, 0.0817795, 0.0322453, 5.00177, 0.208164, 0.995555] 
	...
	saved_FA_param = [
		...
		RhoA_FA_model.GEF0 => saved_FA_parameters[10],
		RhoA_FA_model.GAP0 => saved_FA_parameters[11]
	]
	saved_FA_u0 = [saved_FA_parameters[10:11]; 1]
...
function FA_KO_MSE_loss(new_parameters)
	...
	# Substitute the initial conditions in the parameters
	GEF0_KO = new_parameters[6]
	GAP0_KO = new_parameters[7]
	new_u0 = [GEF0_KO; GAP0_KO ; [1]]
	new_p = new_parameters[1:5]
...
begin
	...
	# Set upper and lower bound for the parameters and initial conditions
	FA_KO_lower_b = repeat([0], 7)
	FA_KO_upper_b = [repeat([10], 5); 2; 2]
...
begin
	...
	saved_FA_KO_param = [
		...
		RhoA_FA_KO_model.GEF0 => saved_FA_KO_parameters[6],
		RhoA_FA_KO_model.GAP0 => saved_FA_KO_parameters[7]]
	saved_FA_KO_u0 = [saved_FA_KO_parameters[6:7]; 1]
